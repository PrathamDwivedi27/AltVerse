...
export default function LoginModal({ isOpen, onOpenChange }) {
  ...
  return (
    <Dialog
      open={isOpen}
      onOpenChange={onOpenChange} // Clicking outside will close
    >
      <DialogContent
        className="sm:max-w-lg p-0 overflow-hidden" // removed default padding for larger image
      >
        <DialogHeader>
          <div className="relative w-full h-64 sm:h-80 mb-4">
            <Image
              src="/AltVerse.jpg"
              alt="A universe in a globe"
              fill={true}
              style={{ objectFit: 'cover' }}
            />
            <div className="absolute inset-0 bg-black/30"></div>
          </div>
          <DialogTitle className="text-center text-2xl font-bold">Join the Adventure</DialogTitle>
          <DialogDescription className="text-center text-muted-foreground">
            Sign in to create, explore, and shape new worlds.
          </DialogDescription>
        </DialogHeader>
